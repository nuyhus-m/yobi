...
//    }
    ...
    @Operation(summary = "OCR 분석하기(저장X)", description = "달력 이미지를 분석합니다. 1일의 요일(whichDay)과 인식된 일정 데이터를 반환합니다.")
    @PostMapping(value = "/ocr/analyze", consumes = MediaType.MULTIPART_FORM_DATA_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    @ApiResponses(value = {
    ...
    public ResponseEntity<?> analyzeScheduleByOcr(
        ...
        ApiResult result = scheduleService.analyzeSchedulesWithOcr(image, year, month, timezone);
    ...
    @Operation(summary = "OCR 분석 결과 저장", description = "OCR 분석 결과를 DB에 저장합니다. 성공한 일정 수(successCount), 실패한 일정 수(failCount), 실패 사유(failureReasons)를 반환합니다.")
    @PostMapping(value = "/ocr/save", consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "OCR 결과 저장 성공",
                    content = @Content(mediaType = "application/json",
                            examples = @ExampleObject(value = "{\"successCount\":10,\"failCount\":2,\"failureReasons\":[\"클라이언트 찾기 실패: '김영희'\",\"시간 중복 일정: 박지성님 (2025-03-09 10:00~11:00)\"]}")))
    })
    public ResponseEntity<?> saveSchedulesByOcr(
            @RequestBody List<OcrDTO.OcrResponseDTO.ScheduleItem> schedules,
            @AuthenticationPrincipal UserDetails userDetails
    ) {
        Integer userId = userDetails != null ?
                authUtils.getUserIdFromUserDetails(userDetails) :
        ...
        ApiResult result = scheduleService.saveOcrSchedules(userId, schedules);
...
//    // 기존 메서드 (하위 호환성 유지)
//    @Operation(summary = "OCR을 이용한 일정 등록(일괄 처리)", description = "이미지, 년, 월을 등록하면 OCR로 분석 후 일정을 자동 등록합니다. <br/>" +
//            "등록 성공한 일정 수(successCount), <br/>" +
//            "양식 오류, 일치 클라이언트 없음 등으로 등록 실패한 일정 수(failCount), <br/>" +
//            "등록 실패 사유(failureReasons)를 반환합니다.")
//    @PostMapping(value = "/ocr", consumes = MediaType.MULTIPART_FORM_DATA_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
